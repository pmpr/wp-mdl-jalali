...
external_jQuery_default()(document).ready(function () {
    ...
    function persian2gregorian(j_y, j_m, j_d)
    {
        j_y = parseInt(j_y);
        j_m = parseInt(j_m);
        j_d = parseInt(j_d);
        ...
        for (var i=0; i < jm; ++i) j_day_no += pDate.p_days_in_month[i];
        ...
        for (var i = 0; g_day_no >= pDate.g_days_in_month[i] + (i == 1 && leap); i++)
            g_day_no -= pDate.g_days_in_month[i] + (i == 1 && leap);
        var gm = i+1;
        var gd = g_day_no+1;
    ...
    function gregorian2persian(g_y, g_m, g_d)
    {
        g_y = parseInt(g_y);
        g_m = parseInt(g_m);
        g_d = parseInt(g_d);
        ...
        for (var i=0; i < gm; ++i)
            g_day_no += pDate.g_days_in_month[i];
        ...
        for (var i = 0; i < 11 && j_day_no >= pDate.p_days_in_month[i]; ++i) {
            j_day_no -= pDate.p_days_in_month[i];
        }
        var jm = i+1;
        var jd = j_day_no+1;
    ...
    external_jQuery_default()('#the-list').on('click', '.editinline', function () {
        ...
        if (year > 1700) {
            ...
            let month = tr.find('.mm').html(),
                ...
                min = tr.find('.mn').html(),
                date = gregorian2persian(year, month, day);
    ...
    if (!PMPRUtil.isEmpty($inlineEditDate)) {
        ...
        $inlineEditDate.on('keyup', '#Jaa , #Jjj', function (e) {
            ...
            let year = external_jQuery_default()('#Jaa').val(),
                ...
                date = persian2gregorian(year, month, day);
    ...
    external_jQuery_default()('a.edit-timestamp').on('click', function () {
        let date = gregorian2persian(external_jQuery_default()('#aa').val(), external_jQuery_default()('#mm').val(), external_jQuery_default()('#jj').val()),
